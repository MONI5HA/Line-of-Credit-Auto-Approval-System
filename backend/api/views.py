...
ml_model_data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'merged_data.csv')
...
class PredictView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Retrieve the applicant's credit score from the dataset (assuming it's stored in a CSV or database)
            applicant_id = data['applicant_id']
            # Here we assume you have a pandas DataFrame `applicant_data` or query to fetch data from the dataset
            # Assuming `applicant_data` is a DataFrame containing the dataset
            applicant_data = pd.read_csv(ml_model_data_path)
  # Adjust to your dataset path
            applicant_row = applicant_data[applicant_data['applicant_id'] == applicant_id]
            
            if applicant_row.empty:
                return Response(
                    {"error": f"Applicant ID {applicant_id} not found in the dataset."},
                    status=status.HTTP_404_NOT_FOUND
                )
            
            # Compare the entered credit score with the stored one
            stored_credit_score = applicant_row['credit_score'].values[0]
            entered_credit_score = data['credit_score']
            
            if stored_credit_score != entered_credit_score:
                return Response(
                    {"error": f"Entered credit score ({entered_credit_score}) does not match stored credit score ({stored_credit_score})."},
                    status=status.HTTP_400_BAD_REQUEST
                )
